...
def generate_subtitles(audio_path: Path) -> list:
    ...
    if not WHISPER_AVAILABLE:
        console.print("[yellow]Whisper no instalado")
        return []
...
def create_video(script_data: dict, audio_path: Path, subtitles: list) -> Path | None:
    ...
    try:
        ...
        console.print("[red]FFmpeg no encontrado. Instalá con: choco install ffmpeg (Windows)")
        return None
    except subprocess.CalledProcessError as e:
        console.print(f"[red]Error FFmpeg: {e.stderr.decode()[:200]}")
        return None
    except Exception as e:
